...
public class TFIDFItemScorer extends AbstractItemScorer {
    ...
    private SparseVector makeUserVector(long user) {
        ...
        //Compute meanRating as μu
        mean = sum/userRatings.size();
        
        for (Rating r: userRatings) {
        	Preference p = r.getPreference();

        	//Create a mutable copy of the item vector. The name of the result vector is weightedItemVector.
        	MutableSparseVector weightedItemVector = model.getItemVector(p.getItemId()).mutableCopy();
        	//Compute weight as rui−u. 
        	weightedItemVector.multiply(p.getValue()-mean);
        	//Add weightedItemVector to profile
